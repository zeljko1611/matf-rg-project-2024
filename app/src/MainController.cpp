...
void MainController::initialize() {
    ...
    m_bloom.initialize(platform_controller->window()->width(), platform_controller->window()->height());
    m_shadow.initialize();
    platform_controller->set_enable_cursor(false);

    auto *resources = Controller::get<engine::resources::ResourcesController>();
    m_asphalt_texture = resources->texture("asphalt");
    m_ferrari_texture = resources->texture("ferrari_paint");
    m_street_lights_texture = resources->texture("street_lights_palette");
    m_formula_model = resources->model("formula");
    m_street_lights_model = resources->model("street_lights");


    m_skybox = resources->skybox("night_sky", "resources/skyboxes/night_sky");
...
bool MainController::loop() {
    return Controller::get<engine::platform::PlatformController>()
            ->key(engine::platform::KEY_ESCAPE)
            .is_up();
}
...
void MainController::poll_events() {
    ...
    if (press(engine::platform::KEY_Z)) m_cone = std::clamp(m_cone - 2.f, 10.f, 32.f);
    if (press(engine::platform::KEY_X)) m_cone = std::clamp(m_cone + 2.f, 10.f, 32.f);
    if (press(engine::platform::KEY_Q)) m_point_intensity = std::clamp(m_point_intensity - 0.2f, 0.f, 3.f);
    if (press(engine::platform::KEY_E)) m_point_intensity = std::clamp(m_point_intensity + 0.2f, 0.f, 3.f);
    if (press(engine::platform::KEY_B)) m_bloom_on = !m_bloom_on;

    if (press(engine::platform::KEY_F1)) {
        m_captured = !m_captured;
        platform_controller->set_enable_cursor(!m_captured);
    }
}
...
void MainController::update() {
    ...
    const float elapsed = platform_controller->frame_time().current - m_action;
    if (m_race == Race::Countdown && elapsed >= 2.f) {
        m_race = Race::Racing;
        spdlog::info("EVENT A: car accelerates.");
    }

    if (m_race == Race::Racing) {
        m_car_x += 5.2f * platform_controller->dt();
        if (elapsed >= 6.f) {
            m_race = Race::Safety;
...
void MainController::begin_draw() {
    ...
    m_shadow.begin_depth_pass(m_point_pos);
    shader->use();
    for (int i = 0; i < 6; ++i)
        shader->set_mat4("shadowMatrices[" + std::to_string(i) + "]", m_shadow.shadow_matrices()[i]);
    shader->set_vec3("lightPos", m_point_pos);
    shader->set_float("farPlane", m_shadow.far_plane());
    scene(shader, true);

    const auto *platform_controller = Controller::get<engine::platform::PlatformController>();
    m_shadow.end_depth_pass(platform_controller->window()->width(), platform_controller->window()->height());
    m_bloom.begin_scene();
}
...
void MainController::draw() {
    ...
    if (m_skybox)
        graphics->draw_skybox(resources->shader("skybox"), m_skybox);

    shader->use();
    shader->set_mat4("projection", graphics->projection_matrix());
    shader->set_mat4("view", graphics->camera()->view_matrix());
    shader->set_vec3("viewPos", graphics->camera()->Position);
    shader->set_vec3("pointPosition", m_point_pos);
    shader->set_vec3("pointColor", {1.f, .72f, .35f});
    shader->set_float("pointIntensity", m_point_intensity);
    shader->set_vec3("spotPosition", {m_car_x + 2.19f, .35f, 0});
    shader->set_vec3("spotDirection", {1, -.07f, 0});
    shader->set_vec3("spotColor", {.76f, .86f, 1});
    shader->set_float("spotCutoff", glm::cos(glm::radians(m_cone)));
    shader->set_float("farPlane", m_shadow.far_plane());
    ...
    m_asphalt_texture->bind_to_unit(2);
    m_street_lights_texture->bind_to_unit(3);
    m_shadow.bind(4);
    scene(shader, false);
}
...
void MainController::end_draw() {
    ...
    m_bloom.compose(blur, final, m_bloom_on, 1.05f);
...
void MainController::start() {
    m_action = Controller::get<engine::platform::PlatformController>()->frame_time().current;
    m_race = Race::Countdown;
    m_car_x = -10;
...
void MainController::formula(const Shader *s, bool d) const {
    glm::mat4 m(1.f);
    m = glm::translate(m, {m_car_x, -.19f, 0.f});
    ...
    if (!d) {
        ...
        m_ferrari_texture->bind_to_unit(0);
    }

    m_formula_model->draw(s);
    box(s, {m_car_x + 2.12f, .34f, -.44f}, {.055f, .035f, .075f}, {.75f, .87f, 1}, 9, d);
    box(s, {m_car_x + 2.12f, .34f, .44f}, {.055f, .035f, .075f}, {.75f, .87f, 1}, 9, d);
...
void MainController::headlights(const engine::resources::Shader *s, bool d, float height,
    ...
    for (float side: {-lateral_offset, lateral_offset}) {
        box(s, {m_car_x + housing_x, height, side}, {.11f, .065f, .115f}, housing_color, 0.f, d);
        box(s, {m_car_x + lens_x, height, side}, {.018f, .038f, .068f}, lens_color, 7.f, d);
    }
}

void MainController::lamp(const engine::resources::Shader *s, float x, float z, bool d) const {
    if (m_street_lights_model) {
        glm::mat4 m(1.f);
        ...
        m_street_lights_model->draw_range(s, 12, 1);
        ...
        m_street_lights_model->draw_range(s, 13, 1);
    }
}
